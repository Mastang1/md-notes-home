...
##### 当前C1的启动链路
 ...
 - 3. 串行启动(flashtool show time)：HSE核心启动、接收flash_loader.bin到ram中；BSE core操作启动cm7_0,跳转到flash_loader.bin执行；进入到flash_tool交互模式，通过发送 命令及参数开启文件传输及数据存储(qspi/sd/emmc)；注意：flash_loader在ram中。
