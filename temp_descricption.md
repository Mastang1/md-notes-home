...
- **Bug修复：** * 重构了 watchdog 三方库的线程处理逻辑，新增重试与退避机制，彻底解决了 SMB 服务异常导致的框架重启问题（本周 0 Crash）。
    
- 为核心日志/报告的 SMB 上传操作增加了限时重试策略，解决网络断开导致的测试文件信息传递失败问题，已通过故障模拟验证。
        
- **框架依赖组件升级：** 成功将 `blob_tool` 升级至 v1.6.2，并完成适配层配置的更新与验证。
    
- **BSE firmware 集成：** 将 BSE firmware 引入 `mcd-test` 测试环境，目前正在 PC_60 平台进行软硬件联调排障，暂未跑通全流程。